...
export const PageGestionUsuarios: React.FC = () => {
  const auth = useAuth();
  const id_empresa = Number(auth?.usuario?.id_empresa);

  const { usuarios, cargando, error, agregarUsuario, editarUsuario, cambiarEstado } = useUsuarios(id_empresa);
  ...
  const handleGuardarUsuario = async (datos: DatosUsuarioFormulario) => {
    if (editingUsuario) {
      const cambios: Partial<DatosUsuarioFormulario> = {};
      
      if (datos.nombres !== editingUsuario.nombres) cambios.nombres = datos.nombres;
      if (datos.apellidos !== editingUsuario.apellidos) cambios.apellidos = datos.apellidos;
      if (datos.email !== editingUsuario.email) cambios.email = datos.email;
      
      const currentTelefono = editingUsuario.telefono || undefined;
      if (datos.telefono !== currentTelefono) cambios.telefono = datos.telefono;

      const rolCambiado = datos.id_rol !== editingUsuario.id_rol ? datos.id_rol : undefined;

      if (Object.keys(cambios).length === 0 && !rolCambiado) {
        showToast('No se detectaron cambios');
        handleCloseModal();
        return;
      }

      const resp = await editarUsuario(editingUsuario.id_usuario, cambios, rolCambiado);
      ...
      const resp = await agregarUsuario({
        ...datos,
        id_empresa
      });
  ...
  const toggleEstado = async (usuario: Usuario) => {
    ...
    const resp = await cambiarEstado(usuario.id_usuario, usuario.estado);
    if (resp.success) {
    ...
    <div className="p-8 max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Administra los accesos, roles e información de los usuarios.</p>
        </div>
      ...
      {error && (
        <div className="p-4 mb-4 bg-red-50 border border-red-200 text-red-700 rounded-xl font-medium">
          ⚠️ {error}
        </div>
      )}
                  ...
                  <tr key={usuario.id_usuario} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-800 border border-gray-200">
                        ...
                        {usuario.rol?.nombre || roles.find(r => r.id_rol === usuario.id_rol)?.nombre || `Rol ${usuario.id_rol}`}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <button
                        ...
                        className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 ${usuario.estado ? 'bg-emerald-500' : 'bg-gray-300'
                          }`}
                      >
                        <span className="sr-only">Cambiar estado</span>
                        <span
                          aria-hidden="true"
                          className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${usuario.estado ? 'translate-x-5' : 'translate-x-0'
                            }`}
                        />
                      </button>
