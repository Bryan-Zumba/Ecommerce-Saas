...
// ─────────────────────────────────────────────
// Modo del panel de clientes
// ─────────────────────────────────────────────
type ClientPanelMode = "idle" | "search_result" | "create" | "edit";
...
function Checkout() {
  ...
  const [clientMode, setClientMode] = useState<ClientPanelMode>("idle");
  const [searchQuery, setSearchQuery] = useState("");
  const [searchError, setSearchError] = useState("");
  const [selectedClient, setSelectedClient] = useState<Client | null>(null);

  // Form temporal para Crear / Editar
  const [clientForm, setClientForm] = useState<Client>({ id: "", nombre: "", email: "", telefono: "" });
  ...
  const vuelto = montoRecibido !== "" ? Math.max(0, montoRecibido - total) : 0;
  const saldoFaltante = montoRecibido !== "" ? Math.max(0, total - montoRecibido) : total;
  ...
  const handleFinalizarVenta = (e: React.FormEvent) => {
    e.preventDefault();
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-6">
                ...
                {clientMode !== "create" && clientMode !== "edit" && (
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 py-2 rounded-xl transition-all active:scale-95 shadow-sm shadow-emerald-200"
                  >
              ...
              {/* ─ Modo: Buscar ─ */}
              {(clientMode === "idle" || clientMode === "search_result") && (
                <div className="space-y-4">
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-4 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition-all bg-gray-50"
                      />
                    </div>
                    <button
                      type="button"
                      onClick={handleSearch}
                      className="bg-gray-800 hover:bg-black text-white text-xs font-bold px-4 rounded-xl transition-all active:scale-95"
                    >
                      Buscar
                    </button>
                  </div>
                  ...
                  {clientMode === "search_result" && selectedClient && (
                    <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center justify-between gap-4">
                      ...
                      <div className="flex flex-col gap-2 shrink-0">
                        <button
                          ...
                          className="text-[11px] font-bold px-3 py-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all"
                        >
                          Seleccionar
                        </button>
                        <button
                          ...
                          className="text-[11px] font-bold px-3 py-1.5 border border-gray-200 text-gray-600 rounded-lg hover:border-gray-400 transition-all"
                        >
                          Editar
                        </button>
                      </div>
              ...
              {(clientMode === "create" || clientMode === "edit") && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2 space-y-1">
                      ...
                      <input
                        type="text"
                        value={clientForm.id}
                        onChange={e => setClientForm(prev => ({ ...prev, id: e.target.value }))}
                        placeholder="17xxxxxxxx"
                        disabled={clientMode === "edit"}
                        className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition-all disabled:bg-gray-50 disabled:text-gray-400"
                      />
                    </div>
                    <div className="col-span-2 space-y-1">
                      ...
                      <input
                        ...
                        onChange={e => setClientForm(prev => ({ ...prev, nombre: e.target.value }))}
                        placeholder="Juan Pérez"
                        className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                      />
                    </div>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button
                      type="button"
                      onClick={handleCancelClientAction}
                      className="px-4 border border-gray-200 text-gray-500 text-sm font-bold rounded-xl hover:border-gray-400 transition-all"
                    >
                      Cancelar
                    </button>
                  </div>
              ...
              <form onSubmit={handleFinalizarVenta} className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1 text-left">
                    ...
                    <input
                      required name="nombre" value={customer.nombre} onChange={handleCustomerInput}
                  ...
                  <div className="space-y-1 text-left">
                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Cédula / RUC</label>
                    <input
                      required name="dni" value={customer.dni} onChange={handleCustomerInput}
                      className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                      placeholder="17xxxxxxxx"
                    />
                  </div>
                  <div className="space-y-1 text-left">
                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Email</label>
                    <input
                      required type="email" name="email" value={customer.email} onChange={handleCustomerInput}
                      className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                  ...
                  <div className="space-y-1 text-left">
                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Teléfono</label>
                    <input
                      name="telefono" value={customer.telefono} onChange={handleCustomerInput}
                      className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                      placeholder="099xxxxxxx"
                    />
                  </div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (val.split('.').length <= 2) {
                            setMontoRecibido(val === "" ? "" : Number(val));
                          }
                        }}
                ...
                <button
                  ...
                  className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-200 disabled:text-gray-400 text-white py-4 rounded-2xl font-bold text-base shadow-lg shadow-emerald-100 transition-all active:scale-95"
                >
